...
def _mla_attn_kernel(
    ...
    cur_batch_actual = cur_batch // S_Q
    cur_head_id = tl.program_id(0)
    split_kv_id = tl.program_id(2)

    cur_batch_seq_len = tl.load(B_seq_len + cur_batch_actual)
    CAUSAL = True
    if CAUSAL:
        q_pos = cur_batch % S_Q
        cur_batch_seq_len = cur_batch_seq_len - S_Q + 1 + q_pos
    ...
    for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
        ...
        kv_page_number = tl.load(
            Req_to_tokens + stride_req_to_tokens_bs * cur_batch_actual + offs_n // PAGE_SIZE,
...
def mla_decode_triton(
    ...
    #num_nan = torch.isnan(attn_logits).sum()
    #print(f"has {num_nan=}")
...
def run_flash_mla_triton(q, block_table, blocked_k, max_seqlen_pad, block_size, b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):
    ...
    def flash_mla_triton():
        ...
        mla_decode_triton(q_nope.view(-1, h_q, dv), q_pe.view(-1, h_q, d-dv), blocked_k_nope.view(-1, dv), blocked_k_pe.view(-1, d-dv), o, block_table, cache_seqlens, attn_logits, num_kv_splits, 1 / math.sqrt(d), block_size, s_q)
...
def compare_ab(baseline, target, b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):
    ...
    close_mask = torch.isclose(out_b.float(), out_a.float(), atol=1e-2, rtol=1e-2)
    if not close_mask.all():
        mismatch_indices = torch.nonzero(~close_mask)
        num_mismatch = mismatch_indices.shape[0]
        total_elems = close_mask.numel()
        print(f"OUTPUT MISMATCH: {num_mismatch}/{total_elems} elements differ ({num_mismatch/total_elems*100:.2f}%)")
        num_to_print = min(num_mismatch, 32) #65536)
        for i in range(num_to_print):
            idx = tuple(mismatch_indices[i].tolist())
            val_a = out_a.float()[idx].item()
            val_b = out_b.float()[idx].item()
            print(f"  idx={idx}: baseline={val_a:.6f}, target={val_b:.6f}, diff={abs(val_b - val_a):.6f}")
        if num_mismatch > num_to_print:
            print(f"  ... and {num_mismatch - num_to_print} more mismatches")
...
shape_configs = [
    {"b": batch, "s_q": 4, "cache_seqlens": torch.tensor([seqlen + 0 * i for i in range(batch)], dtype=torch.int32, device="cuda"), "h_q": head, "h_kv": 1, "d": 512+64, "dv": 512, "causal": True, "dtype": torch.bfloat16}
    for batch in [128] for seqlen in [10000] for head in [128]
]
